...
export class RentCastService {
   ...
   */
  async fetchPropertyFromRentCast(address: string): Promise<any> {
   ...
   */
  transformRentCastResponse(property: any) {
    // Get the latest tax assessment
    const latestAssessmentYear = property.taxAssessments
      ? Math.max(...Object.keys(property.taxAssessments).map(Number))
      : null;
    const latestAssessment = latestAssessmentYear
      ? property.taxAssessments[latestAssessmentYear]
      : null;

    // Get the latest property tax
    const latestTaxYear = property.propertyTaxes
      ? Math.max(...Object.keys(property.propertyTaxes).map(Number))
      : null;
